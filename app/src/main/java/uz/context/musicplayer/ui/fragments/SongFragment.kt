...
import uz.context.musicplayer.databinding.FragmentSongBinding
...
import uz.context.musicplayer.ui.viewmodels.SongViewModel
import uz.context.musicplayer.util.Status.SUCCESS
...
class SongFragment : Fragment() {
    ...
    private var curPlaySong: Song? = null
    ...
    private fun initViews() {
        ...
        binding.seekBar.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            override fun onProgressChanged(seekBar: SeekBar?, progress: Int, fromUser: Boolean) {
                if (fromUser) {
                    setCurPlayTimeToTextView(progress.toLong())
                }
            }
    ...
    private fun subscribeToObservers() {
                ...
                when (result.status) {
                    SUCCESS -> {
                        result.data?.let { songs ->
                            if (curPlaySong == null && songs.isNotEmpty()) {
                                curPlaySong = songs[0]
        ...
        mainViewModel.curPlaySong.observe(viewLifecycleOwner) {
            ...
            curPlaySong = it.toSong()
            updateTitleAndSongImage(curPlaySong!!)
        }
    ...
    private fun setCurPlayTimeToTextView(ms: Long) {
        val dateFormat = SimpleDateFormat("mm:ss", Locale.getDefault())
